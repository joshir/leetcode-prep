...
public class NonCoincidingPairs {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
      numNonCoincidingPairs( new int[] {4, 2, 4, 6, 2, 8, 4, 2}, new int[] {3,3,6,1,3,3,6,3}, 8));
  }
  ...
  public static int numNonCoincidingPairs(int[] xs, int[] ys, int N) {
    ...
    for (int index = 0; index < N; index++){
      Pair currentPair = new Pair(xs[index],ys[index]);
      pairs = cxMap.getOrDefault(xs[index], 0) +
              cyMap.getOrDefault(ys[index],0) -
              2 * coordinateMap.getOrDefault(currentPair,0);
